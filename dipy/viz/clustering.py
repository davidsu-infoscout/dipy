...
def show_clusters_graph(tree, bg=(1, 1, 1), show=False, show_id=False, linewidth_streamlines=1., linewidth_edges=1.):
    import networkx as nx
    ...
    def _draw_subtree(node):
        ...
        if node.color is not None:
            ...
            stream_actor = actor.streamtube([s - mean + node_pos for s in node], [node.color]*len(node),
                                            linewidth=linewidth_streamlines)
    ...
    #renderer.add(actor.line(lines[0], colors.black, linewidth=1))
    renderer.add(actor.streamtube(lines[0], colors.grey, linewidth=linewidth_edges, opacity=0.6))
...
def show_clusters_tree_path(tree, path, bg=(1, 1, 1), show=False,
                            show_id=False,
                            linewidth_streamlines=1.,
                            linewidth_edges=1.):
    import networkx as nx
    ...
    def _draw_subtree(node):
        ...
        if node.color is not None:
            ...
            stream_actor = actor.streamtube([s - mean + node_pos for s in node], [color]*len(node),
                                            linewidth=linewidth_streamlines)
            renderer.add(stream_actor)
    ...
    renderer.add(actor.streamtube(lines[0], colors.grey, linewidth=linewidth_edges, opacity=1))
